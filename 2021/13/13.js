...
const input = fs.readFileSync(path.resolve(__dirname, '13.txt'))
    .toString()
    .split('\n');
    ...
    if(fold[0] == 'y'){
        for(let y=start+1;y<map.length;y++){
            for(let x=0;x<map[0].length;x++){
                map[start-(y-start)][x]=map[start-(y-start)][x] == '#' ? '#' : map[y][x];
            }
        }
        ...
        for(let y=0;y<map.length;y++){
            for(let x=start+1;x<map[0].length;x++){
                map[y][start-(x-start)]=map[y][start-(x-start)] == '#' ? '#' : map[y][x];
            }
        }
        let result=[];
        for(let x of map){
